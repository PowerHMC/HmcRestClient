...
class PowerOffPartition(JobStatus):
    ...
    def poweroff_Partition(self, ip, logicalpartition_object, session_id):
        """
        performs the poweroff operation for the provided LogicalPartition object
        Args:
           ip : ip address of HMC
           logicalpartition_object : object of logicalpartition to be deactivated
           session_id : session to be used
        """
        super().__init__(ip, self.root, self.content_type, session_id)
        log_object.log_debug("power off of lpar started")
        ...
        request_object.HTTPPut(payload=self.input, append=str(logicalpartition_uuid)+"/do/PowerOff")
        log_object.log_debug("response of power off job request --- %s"%(request_object.response))
